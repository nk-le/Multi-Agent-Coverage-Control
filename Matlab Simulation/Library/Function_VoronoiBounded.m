...
function [V,C]= Function_VoronoiBounded(x,y, crs)
    ...
    bndPoly = polyshape(crs(:,1),crs(:,2), 'Simplify', false);
    for ij=1:length(C)
            ...
            [X2, Y2] = sort_poly_cw(V(C{ij},1), V(C{ij},2));
            A = polyshape(X2,Y2, 'Simplify', false);
            partition = intersect(A, bndPoly);
            cleanP = rmslivers(partition,1e-6);

            [xb, yb] = boundary(cleanP);
