...
% 
%  READMTX will be removed in a future release.
%
...
case {'binary', 'formatted, fixed length'}
    ...
    if (expectedfilesize ~= eof)
        fclose(fid);
        error(message('map:fileio:inconsistentFileSize', num2str( expectedfilesize ), num2str( eof )))
    end
end
...
if ((rowstep == 1 && colstep == 1) || ... 							    % every value
	...
	&& isempty(strfind(precision, '%'))									% not a formatted (fscanf) read
    ...
    switch datatype
        ...
        case 'formatted, fixed length'
            if ~isempty(strfind(precision, 'D'))
                fortranformat = 1;
            ...
            for i=1:nrowread
                ...
                if count ~= recordlen-nRowHeadBytes-nRowTrailBytes
                    fclose(fid);
		              error(message('map:fileio:unexpectedElementCount',   ...
                       recordlen-nRowHeadBytes-nRowTrailBytes, count));
                end
                ...
                if count ~= ncols
                    fclose(fid);
                    error(message('map:fileio:unexpectedStringElementCount',  ...
                       ncols, count , '%12g'))
                end
        ...
        case 'formatted, variable length' 		
            ...
            for i=1:nrows                
                ...
                if ismember(i,readrows)                   
                    ...
                    if fortranformat					
                        % extension to C conversion characters for Fortran 0.0D00 data
                        str = strrep(str,'D','e');
                    end
...
if ~isempty(strfind(precision, '%s'))	&& ...
        (~isempty(strfind(precision,'c')) || ~isempty(strfind(precision, 's')))
    data = char(data);
end
...
otherwise
	...
	if ~isempty(strfind(precision,'%')) % SCANF-style formatted data ('%5d')
		...
		fclose(fid);
		error(message('map:fileio:invalidFieldType', precision));
	end
end
