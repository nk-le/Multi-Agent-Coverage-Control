...
%  
%      Filename     A character array containing the names of the files 
%                   that were read
%
%      ShapeType    A character vector containing the shape type
%
...
%
%                   Name   A character vector containing the attribute name 
%                          as given in the xBASE table
%
%                   Type   A character vector specifying the MATLAB class 
%                          of the attribute data returned by SHAPEREAD. The
...
[shpFileId, shxFileId, dbfFileId, headerTypeCode, prjWKT] = ...
...
% If available, add CRS.
if ~isempty(prjWKT)
    try
        if startsWith(prjWKT,'GEO','IgnoreCase',true)
            crs = geocrs(prjWKT);
        else
            crs = projcrs(prjWKT);
        end
        INFO.CoordinateReferenceSystem = crs;
    catch
    end
end
