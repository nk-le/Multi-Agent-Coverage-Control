...
%
%   FILENAME is a character vector or string scalar that specifies the
%   output file name and location. If your FILENAME includes an extension,
%   it must be '.tif' or '.TIF'. The output file is a tiled GeoTIFF file if
%   the input, A, is at least 160-by-160 in size. Otherwise, it is
%   organized as rows-per-strip.
%
...
%                            and you can use a string scalar or character
%                            vector, such as, 'EPSG:4326'. If you specify
%                            the coordinate system with a string scalar or
%                            character vector, include the 'EPSG:' prefix.
%                            See the GeoTIFF specification or the EPSG data
%                            files (pcs.csv and gcs.csv) for the code
%                            numbers.
%
...
%
%   'TiffType'               Scalar string or character vector that
%                            specifies the type of TIFF file. Permissible
%                            values are 'classictiff' which specifies to
%                            write a ClassicTIFF file, or 'bigtiff' which
%                            specifies to write a BigTIFF file. The default
%                            value is 'classictiff'.
%
...
%   Example 2
%   ---------
%   % Convert a ClassicTIFF GeoTIFF file to a tiled BigTIFF file.
...
%   layer = refine(nasa,layerName,'SearchField','layername', ...
%        'MatchType','exact');
%   [A,R] = wmsread(layer(1));
...
%   Example 5
%   ---------
%   % Write a subimage from a GeoTIFF file to a new GeoTIFF file.
%   [A,RA] = readgeoraster('boston.tif');
%   xlimits = [ 776770   778835];
%   ylimits = [2956260  2957700];
%   [B,RB] = mapcrop(A,RA,xlimits,ylimits);
%   info = geotiffinfo('boston.tif');
%   filename = 'boston_subimage.tif';
%   geotiffwrite(filename,B,RB, ...
...
%   Example 6
%   ---------
%   % Write elevation data for an area around South Boulder Peak in
%   % Colorado to a GeoTIFF file. First, import the elevation data and a
%   % geographic postings reference object.
%   [Z,R] = readgeoraster('n39_w106_3arc_v2.dt1','OutputType','double');
%
%   % Specify GeoKey directory tag information as a structure.
%   key.GTModelTypeGeoKey  = 2;  % Coordinate system type (geographic)
%   key.GTRasterTypeGeoKey = 2;  % PixelIsPoint (postings)
%   key.GeographicTypeGeoKey = 4326;   % Geographic coordinate system
%
...
%   demcmap(g.CData)
%
%   % The elevation data used in this example is courtesy of the U.S.
%   % Geological Survey.
%
%   See also GEOTIFFINFO, IMREAD, IMWRITE, map.geotiff.RPCCoefficientTag, READGEORASTER, Tiff
...
rpcTag = Params.RPCCoefficientTag;
isClassicTIFF = Params.IsClassicTIFF;
writeGeoTiffFile(filename, A, TiffTags, GeoTiffTags, rpcTag, isClassicTIFF);
...
if isempty(cmap)
    ...
    validateattributes(A, classes, {'nonempty'}, mfilename, 'A', 2);
else
    % M-by-N indexed image
    classes = {'uint8','uint16'};
    validateattributes(A, classes, {'nonempty'}, mfilename, 'X', 2);
...
% returned containing the field names:
%    'TiffTags', 'GeoKeyDirectoryTag', 'RPCCoefficientTag', 'IsClassicTIFF'
%
...
% Validate TiffType, if present.
paramName = 'TiffType';
validOptions = {'classictiff', 'bigtiff'};
defaultOption = validOptions{1};
tiffType = S.TiffType;
if ~isempty(tiffType)
    tiffType = validatestring(tiffType, validOptions, mfilename, paramName);
else
    tiffType = defaultOption;
end

 Params.IsClassicTIFF = strcmp(tiffType, defaultOption);
...
if ~isempty(cmap)
    ...
    numBands = size(A, 3);
    isRGB = ndims(A) == 3 && numBands == 3 ...
        && any(strcmp(class(A), {'uint8', 'uint16', 'uint32'}));
    if isRGB
        photometric = Tiff.Photometric.RGB;
    else
        photometric = Tiff.Photometric.MinIsBlack;
        if numBands > 1 && ~islogical(A)
            % Signed datatypes require MinIsBlack Photometric
            % Interpolation. SamplesPerPixel is numBands, but MinIsBlack
            % corresponds to only one sample. Create a [1xnumBands-1] array
            % of EmptySamples to represent the remaining samples.
            extraSamples = Tiff.ExtraSamples.Unspecified;
            n = max(1, numBands-1);
            extraSamples(1:n) = extraSamples(1);
            TiffTags.ExtraSamples = extraSamples;
        end
    end
end
...
if ~islogical(A)
    ...
    if ~ismatrix(A)
        % Only one logical channel is supported by Tiff.
        % Use same message as imwrite.
...
function writeGeoTiffFile(filename, A, TiffTags, GeoTiffTags, rpcTag, isClassicTIFF)

 % Assign writeMode based on TIFF type.
if isClassicTIFF
   writeMode = 'w';
else
    writeMode = 'w8';
...
% Set the tags and write image data.f
setTag(tiffObj, TiffTags);
setTag(tiffObj, GeoTiffTags);

if isTiled(tiffObj) && ~isClassicTIFF
    % Tiled and BigTIFF
    writeImageAsTiles(tiffObj, A)
else
    writeImage(tiffObj, A, isClassicTIFF)
end

% Add RPCCoefficientTag if not empty.
if ~isempty(rpcTag)
    tagValue = double(rpcTag);
    map.geotiff.internal.writeRPCCoefficientTag(filename, tagValue);
end

%--------------------------------------------------------------------------

function writeImage(tiffObj, A, isClassicTIFF)
% Write image A to TIFF file.

% In case of error, or when terminating the function, close the TIFF file.
closeObj = onCleanup(@()close(tiffObj));

try
    write(tiffObj, A);
catch eTIFF
    w = warning('off','backtrace');
    warnObj = onCleanup(@()warning(w));
    if isClassicTIFF ...
            && strcmp(eTIFF.identifier, 'MATLAB:imagesci:Tiff:tileWritefailure')
        eBigTIFF = MException( ...
            message('map:geotiff:considerBigTIFF', 'TiffType', 'bigtiff'));
        eBigTIFF = addCause(eBigTIFF, eTIFF);
        throwAsCaller(eBigTIFF)
    else
        throwAsCaller(eTIFF)
    end
end
...
function writeImageAsTiles(tiffObj, A)
% Write image A to tiled TIFF file.

% In case of error, or when terminating the function, close the TIFF file.
closeObj = onCleanup(@()close(tiffObj));

% Write image data in tiles.
try
    numRows = getTag(tiffObj, 'ImageLength');
    numCols = getTag(tiffObj, 'ImageWidth');
    numRowsInTile = getTag(tiffObj, 'TileLength');
    numColsInTile = getTag(tiffObj, 'TileWidth');
    
    % If status is desired, uncomment these lines:
    % numTiles = numberOfTiles(tiffObj);
    % tile = 0;
    
    for startCol = 1:numColsInTile:numCols
        for startRow = 1:numRowsInTile:numRows
            tileNumber = computeTile(tiffObj, [startRow, startCol]);
            endRow = min(startRow + numRowsInTile - 1 ,numRows);
            endCol = min(startCol + numColsInTile - 1, numCols);
            imageData = extractTile(A, startRow, endRow, startCol, endCol);
            writeEncodedTile(tiffObj, tileNumber, imageData);
            
            % If status is desired, uncomment these lines:
            % tile = tile + 1;
            % fprintf('Tile number: %d (%d/%d)\n', tileNumber, tile, numTiles)
        end
    end            
catch e
    throwAsCaller(e)
...
function imageData = extractTile(A, startRow, endRow, startCol, endCol)
% Extract rows and columns from A. 

imageData = A(startRow:endRow, startCol:endCol, :);

% Note: In the future if A is tall, use gather:
% if isTall(A)
%    imageData = gather(A(startRow:endRow, startCol:endCol, :));
% else
%   imageData = A(startRow:endRow, startCol:endCol, :);
% end
