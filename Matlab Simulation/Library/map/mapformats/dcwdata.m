...
%
%   DCWDATA will be removed in a future release. The VMAP0 dataset has
%   replaced DCW and can be accessed using VMAP0DATA.
%
...
% Warn that this function is being removed. This function calls dcwread.
% Turn off the warning for dcwread until this function ends to prevent
% multiple warning messages.
warning(message('map:removing:dcwdata','DCWDATA'))
w = warning('off', 'map:removing:dcwread');
...
elseif ~isequal(sort(size(latlim)),[1 2])
    error('map:dcwdata:invalidLatLimitSize', ...
        'Latitude limit input must be a scalar or 2 element vector')
end

if isequal(size(lonlim),[1 1])
   lonlim = lonlim*[1 1];
elseif ~isequal(sort(size(lonlim)),[1 2])
    error('map:dcwdata:invalidLonLimitSize', ...
        'Longitude limit input must be a scalar or 2 element vector')
end
...
if ischar(topolevel)
   ...
   if isempty(topoindx)
      error('map:dcwdata:invalidTOPOLEVEL', ...
          'Topology level is not ''patch'', ''line'', ''point'' or ''text'' ');
   end
   if nargout ~= 1
      error('map:dcwdata:invalidOutputArgumentCount', ...
          'Number of output arguments doesn''t match requested topology levels');
   end
...
elseif iscell(topolevel)
   ...
   for i=1:length(topolevel)
      if ~ischar(topolevel{i})
         error('map:dcwdata:invalidTOPOLEVEL', ...
             'Topology level must be a cell array of strings');
      end
      thistopoindx = strmatch(topolevel{i},{'patch','line','point','text'},'exact');
      if isempty(thistopoindx)
         error('map:dcwdata:invalidTOPOLEVELTYPE', ...
             'Topology level must be ''patch'', ''line'', ''point'' or ''text'' ');
      end
   ...
   if length(topoindx) ~=length(topolevel)
      error('map:dcwdata:invalidTOTOREQUESTS', ...
          'Redundant requests in topology levels')
   elseif nargout ~= length(topoindx)
      error('map:dcwdata:invalidNumberOfOutputs', ...
          'Number of outputs doesn''t match requested topology levels')
   end
else
   error('map:dcwdata:invalidTOPOLEVEL', ...
       'Topology level must be a string or cell array of strings')
end % if
...
if isempty(strmatch('DCW',upper(strvcat(dirstruc.name)),'exact'))
   error('map:dcwdata:deviceError', ...
       ['DCW disk ' upper(library) ' not mounted or incorrect devicename ']); 
end
...
if isempty(strmatch(theme,{CAT.COVERAGE_NAME},'exact'))
   ...
   error('map:dcwdata:invalidLibrary', ...
       ['Theme not present in library ' library char(linebreak) char(linebreak) ...
       'Valid two-letter theme identifiers are: ' char(linebreak) goodthemes ])
end
      ...
      case 1
         if any(topoindx ==1)
            ...
            if ~(any(strcmp(names,FTfilename))==1)
               warning('map:dcwdata:noPatchData', ...
                   'No patch data for this theme on this tile')
...
%
      case 2
         ...
         if any(topoindx ==2)
            ...
            if ~(any(strcmp(names,FTfilename))==1)
               warning('map:dcwdata:noLineData', ...
                   'No line data for this theme on this tile')
            elseif ~(any(strcmp(tilenames,EntityName))==1)
               warning('map:dcwdata:noLineData', ...
                   'No line data for this theme on this tile')
            else
      ...
      case 3
         ...
         if any(topoindx ==3)
            ...
            if ~(any(strcmp(names,FTfilename))==1)
               warning('map:dcwdata:noPointData', ...
                   'No point data for this theme on this tile')
            elseif ~(any(strcmp(tilenames,EntityName))==1)
               warning('map:dcwdata:noPointData', ...
                   'No point data for this theme on this tile')
            else
      ...
      otherwise
         ...
         if any(topoindx ==4)
            ...
            if ~(any(strcmp(names,FTfilename))==1)
               warning('map:dcwdata:noTextData', ...
                   'No text data for this theme on this tile')
            elseif ~(any(strcmp(tilenames,EntityName))==1)
               warning('map:dcwdata:noTextData', ...
                   'No text data for this theme on this tile')
            else
...
for i=1:length(ringptrs)
   ...
   if checkface ~= facenum
      warning('map:dcwdata:inconsistentFaceAndRingTable', ...
...
if nargin ~= 3
   error('map:dcwdata:incorrectArgumentCount','Incorrect number of arguments')
end
...
elseif EDG(startedge).LEFT_FACE(1) == facenum
        ...
        warning('map:dcwdata:unexpectedContentFollowingFace', 'Error following face.');
        return
end % if
...
% Breaking tie with nodes
                        ...
                        elseif ~isempty(starttest) & isempty(endtest)
                                ...
                                warning('map:dcwdata:unexpectedContentFollowingFace', ...
                                    'Error following face..')
                        return
...
elseif nargin ~= 7
   error('map:dcwdata:argumentCount','Incorrect number of arguments')
end
...
if length(ET) ~= length(FT)'
   error('map:dcwdata:inconsistentFeatureAndEntityTables', ...
       'Feature table and entity table are different lengths'); 
end
...
if length(FAC) ~= length(FT)'
   warning('map:dcwdata:inconsistentFeatureAndEntityTables', ...
       'Feature table and entity table are different lengths'); 
   return;
end
...
%
   ...
   nprop = fvsz(2);
   if nprop > 1
      warning('map:dcwdata:ignoringMultipleIntegerValues', ...
          'Cannot import with more than 1 integer value field')
      return
   end
...
if length(ET) ~= length(FT)'
   error('map:dcwdata:inconsistentFeatureAndEntityTables', ...
       'Feature table and entity table different lengths'); 
end
...
if isempty(FTindx)
   warning('map:dcwdata:missingFeatureTableEntry', ...
       'No appropriate Feature Table entry in Value Description Table')
   return
end
