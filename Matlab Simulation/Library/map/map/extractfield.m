...
%                   N = numel([S(:).(name)]).
%
%   NAME is a case-sensitive character vector or string scalar defining the 
%   field name of the structure S.  A will be a cell array if any field 
%   values in the fieldname contain text or if the field values are not 
%   uniform in type; otherwise A will be the same type as the field values.
%   The shape of the input field is not preserved in A.
%   
%   Examples
%   --------
...
validateattributes(name, {'char', 'string'}, {'scalartext'}, fcnname, 'NAME', 2)
