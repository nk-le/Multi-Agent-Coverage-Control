...
%   object, or a vector of the form [semimajor_axis, eccentricity]. LATIN
%   is a array of input latitude values. FROM and TO are each one of the
%   latitude types listed below (or unambiguous abbreviations).
%   LATIN has the angle units specified by UNITS: either 'degrees',
%   'radians', or unambiguous abbreviations. The output array, LATOUT, has
%   the same size and units as LATIN.
%
...
%            parametricLatitude, geodeticLatitudeFromParametric,
...
if strcmp(units,'nocheck')
    ...
    % except for the ellipsoid input.
    lat = latin;
...
validateattributes(latin,{'double'},{'real'}, mfilename, 'LATIN', 2);
from  = validatestring(from, lattypes, mfilename, 'FROM', 3);
to    = validatestring(to,   lattypes, mfilename, 'TO', 4);
units = validatestring(units,{'degrees','radians'}, mfilename, 'UNITS', 5);
...
%--------------------------------------------------------------------------
function phi = geocentric2geodetic(e,phi_g,~)
phi = atan2( sin(phi_g), (1-e^2)*cos(phi_g) );

%--------------------------------------------------------------------------
function phi_g = geodetic2geocentric(e,phi,~)
phi_g = atan2( (1-e^2)*sin(phi), cos(phi) );

%--------------------------------------------------------------------------
function latout = parametric2geodetic(e,latin,~)
latout = atan2( sin(latin), sqrt(1-e^2)*cos(latin) );

%--------------------------------------------------------------------------
function latout = geodetic2parametric(e,latin,~)
...
%--------------------------------------------------------------------------
function chi = geodetic2conformal(ecc,phi,~)
...
if ecc > 0.5
    warning('map:convertlat:highEccentricity', ...
        'Auxiliary sphere approximation weakens with eccentricity > 0.5');
end
