...
case 'on'
	...
	for i=1:nparallels
		...
		[x,y,~,savepts] = map.crs.internal.mfwdtran(mstruct,lat,lon,[],'line');
	...
	uistack(hline,'top') % ensure top of stacking order so we can grab it
...
case 'linemove'
	...
	% convert to lat,lon
	[latd,lond] = map.crs.internal.minvtran(mstruct,cpoint(1,1),cpoint(1,2),1);
