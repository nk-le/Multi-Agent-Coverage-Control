...
%  Extended click moves the marker along the map X or Y direction
...
case 'on'          %  Turn origin marker on
    ...
    if isempty(mstruct)
	 ...
	 [x,y]  = map.crs.internal.mfwdtran(mstruct,origin(1),origin(2),0,'none');
...
%  Add the origin text string
     ...
     uicontrol(gcf,'Style','text', 'String',textstr,...
              'Units','Points',  'Position',[2 2 300 20], ...
...
case 'key'             %  Key strokes recognized
 ...
 case 'apply'      %  Reproject the map
	 ...
	 [lat,lon] = map.crs.internal.minvtran(x,y,0);
     setm(gca,'Origin',[round([lat lon], 2) oldor(3)] );
     originui('on');
...
%  A call to minvtran is time consuming.  Vectorize to increase efficiency.
	 ...
	 [lats,lons] = map.crs.internal.minvtran(mstruct,[x;pt(1,1)],[y;pt(1,2)],0);
...
case 'keyorigin'             %  Move origin by keystroke
	 ...
	 [lat,lon] = map.crs.internal.minvtran(mstruct,x,y,0);
     ...
     [x,y] = map.crs.internal.mfwdtran(mstruct,lat,lon,0,'none');
