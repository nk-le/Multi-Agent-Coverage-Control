...
%
%   TRACKUI(AX) activates the interactive tool and associates it with
...
set(h.lonORrnglabel,'String','Rng', ...
    'CallBack',@(hsrc,~) expandedEdit(hsrc,'Range'))
set(h.endselect,'String','Range Units', ...
...
set(h.endORdirtitle, 'TooltipString', tooltipHelpStrings('TrackDirection'))
...
set(h.latORazlabel,'String','Lat', ...
    'CallBack',@(hsrc,~) expandedEdit(hsrc,'Ending Latitude'))
set(h.lonORrnglabel,'String','Lon', ...
    'CallBack',@(hsrc,~) expandedEdit(hsrc,'Ending Longitude'))
...
set(h.endORdirtitle, 'TooltipString', tooltipHelpStrings('TrackEnd'))
set(h.endselect,     'TooltipString', tooltipHelpStrings('MouseSelect'))

%--------------------------------------------------------------------------

function apply_cb(uiFigure)
%  Apply the currently defined track to the map
...
if get(h.twopt,'Value')   %  Two point track definition
    try
        [lat,lon] = track2(trackstr, ...
            str2double(startlat),str2double(startlon),...
            str2double(latORaz), str2double(lonORrng),...
    ...
    if strcmp(unitstr,'Radians')
        dist = str2double(lonORrng);
    else
        dist = str2double(lonORrng) * unitsratio('kilometers',unitstr);
    end
    
    try
        [lat,lon] = track1(trackstr, ...
            str2double(startlat), str2double(startlon), ...
            str2double(latORaz),  dist*radfact, mapstruct.geoid, 'degrees');
    catch e
        uiwait(errordlg(e.message,'Track Error','modal'))
        return
    end
...
% TRACKUIBOX Construct the trackui GUI
...
h.figure =  figure('Units','points',  'Position',[40 80 280 350], ...
	'NumberTitle','off', 'Name','Define Tracks','MenuBar','none', ...
...
h.stlatlabel = uicontrol(h.figure, 'Style','push', 'String', 'Lat', ...
	...
	'FontSize',10,  'FontWeight','bold', 'HorizontalAlignment','left', ...
	'Interruptible','on',...
	'CallBack',@(hsrc,~) expandedEdit(hsrc,'Starting Latitude'));
...
h.stlonlabel = uicontrol(h.figure, 'Style','push', 'String', 'Lon', ...
	...
	'FontSize',10,  'FontWeight','bold', 'HorizontalAlignment','left', ...
	'Interruptible','on',...
	'CallBack',@(hsrc,~) expandedEdit(hsrc,'Starting Longitude'));
...
h.endselect = uicontrol(h.figure, 'Style','push', 'String', 'Mouse Select', ...
	...
	'BackgroundColor',frameclr, 'ForegroundColor','black', ...
	'FontSize',10,  'FontWeight','bold', ...
	'UserData',[h.latORaz h.lonORrng], 'Callback', @(~,~) select_cb(h.figure));
...
set(h.styletext, 'TooltipString', tooltipHelpStrings('TrackStyle'))
set(h.modetext,  'TooltipString', tooltipHelpStrings('TrackMode'))
set(h.anglabel,  'TooltipString', tooltipHelpStrings('AngleLabel'))

set(h.sttitle,   'TooltipString', tooltipHelpStrings('TrackStart'))
...
set(h.latORazlabel,  'TooltipString', tooltipHelpStrings('BigEdit'))
set(h.lonORrnglabel, 'TooltipString', tooltipHelpStrings('BigEdit'))
...
set(h.help,      'TooltipString', tooltipHelpStrings('trackuiHelp'))
