...
case 'initialize'
	...
	uiwait(msgbox('Click on center and perimeter to define sector','modal'))
	% geographic data
...
case 'reproject'
	...
	for i = 1:length(obj)
		...
		% sector data
		mat = [lat1(:) lon1(:); lat2(:) lon2(:); lat3(:) lon3(:)];
		[x,y,z,t] = map.crs.internal.mfwdtran(mat(:,1),mat(:,2),zeros(size(mat(:,1))),'line');
		s.clipped = t.clipped;
		s.trimmed = t.trimmed;
	...
	for i = 1:length(obj)
		s = get(obj(i),'userdata');
		[x,y,z] = map.crs.internal.mfwdtran(s.clat,s.clon,zeros(size(s.clat)));
		% slip in the x and y data
	...
	for i = 1:length(obj)
		s = get(obj(i),'userdata');
		[x,y,z] = map.crs.internal.mfwdtran(s.slat,s.slon,zeros(size(s.slat)));
		% slip in the x and y data
		set(obj(i),'xdata',x,'ydata',y,'zdata',z);
	end	
	% radius
	...
	for i = 1:length(obj)
		s = get(obj(i),'userdata');
		[x,y,z] = map.crs.internal.mfwdtran(s.rlat,s.rlon,zeros(size(s.rlat)));
		% slip in the x and y data
		set(obj(i),'xdata',x,'ydata',y,'zdata',z);
	end	
	% rotate
	...
	for i = 1:length(obj)
		s = get(obj(i),'userdata');
		[x,y,z] = map.crs.internal.mfwdtran(s.elat,s.elon,zeros(size(s.elat)));
		% slip in the x and y data
		set(obj(i),'xdata',x,'ydata',y,'zdata',z);
	end	
	% center azimuth
	...
	for i = 1:length(obj)
		...
		[cazlat,cazlon] = track2(s.trk,s.clat,s.clon,s.rlat,s.rlon,[1 0],...
							 'deg',s.npts/3);
		[x,y,z] = map.crs.internal.mfwdtran(cazlat,cazlon,zeros(size(cazlat))); 
		% slip in the x and y data
		set(obj(i),'xdata',x,'ydata',y,'zdata',z);
	end	
...
case 'centermove'
	...
	cy = cpoint(1,2);
	cz = 0;
	[clat,clon] = map.crs.internal.minvtran(cx,cy);	
	...
	% plot sector
	mat = [lat1(:) lon1(:); lat2(:) lon2(:); lat3(:) lon3(:)];
	[sectx,secty,sectz,t] = map.crs.internal.mfwdtran(mat(:,1),mat(:,2),zeros(size(mat(:,1))),'line');
	s.clipped = t.clipped;
	s.trimmed = t.trimmed;
...
case 'resizemove'
	...
	cy = cpoint(1,2);
	cz = 0;
	[slat,slon] = map.crs.internal.minvtran(cx,cy); 	
	% update center control point
	[x,y,z] = map.crs.internal.mfwdtran(s.clat,s.clon,zeros(size(s.clat))); 
...
case 'radiusmove'
	...
	cy = cpoint(1,2);
	cz = 0;
	[rlat,rlon] = map.crs.internal.minvtran(cx,cy); s.rlat = rlat; s.rlon = rlon;
	% new radius
...
case 'rotatemove'
	...
	cy = cpoint(1,2);
	cz = 0;
	[elat,elon] = map.crs.internal.minvtran(cx,cy); 
