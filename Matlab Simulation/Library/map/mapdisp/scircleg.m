...
%  any valid LineSpec to display the small circles.
%
...
%
%  SCIRCLEG('track',...) uses the track input to define
...
if isempty(ncirc)
    ...
    validateattributes(ncirc, {'double'}, ...
        {'real','positive','scalar','integer'}, 'SCIRCLEG', 'N')
end
...
if isempty(str)
    ...
    str = validatestring(str, {'gc','rh'}, 'SCIRCLEG', 'TRACK', 1);
end
...
if isobject(geoid)
    geoid = [geoid.SemimajorAxis geoid.Eccentricity];
end
...
case 'reproject'
	...
	if ~isempty(hcirc)
		for i = 1:length(hcirc)
			...
			[crlat,crlon] = scircle2(s.trk,s.clat,s.clon,s.rlat,s.rlon,[1 0],...
									 'deg',s.npts);
			% update the track 
			[crx,cry,crz,t] = map.crs.internal.mfwdtran(crlat,crlon,zeros(size(crlat)),'line');
			s.clipped = t.clipped;
			s.trimmed = t.trimmed;
	...
	if ~isempty(hcenter)
		for i = 1:length(hcenter)
			...
			[x,y] = map.crs.internal.mfwdtran(s.clat,s.clon); z = 0;
			% slip in the x and y data
	...
	if ~isempty(hend)
		for i = 1:length(hend)
			...
			[x,y] = map.crs.internal.mfwdtran(rlat,rlon); z = 0;
			% slip in the x and y data
...
case 'npts'
	...
	[crlat,crlon] = scircle2(s.trk,s.clat,s.clon,s.rlat,s.rlon,[1 0],...
							'deg',s.npts);
	% geographic to map coordinates
	[x,y] = map.crs.internal.mfwdtran(s.clat,s.clon);	z = zeros(size(x));
	[rx,ry] = map.crs.internal.mfwdtran(s.rlat,s.rlon);	rz = zeros(size(rx));
	[crx,cry,crz,t] = map.crs.internal.mfwdtran(crlat,crlon,zeros(size(crlat)),'line');
	s.clipped = t.clipped;
	s.trimmed = t.trimmed;
	% slip in the x and y data
...
case 'changeunits'	
	...
	[lat,lon] = map.crs.internal.minvtran(x,y);
	% radial point
	hends = findobj(gca,'tag','InteractiveCircleEnd');
	for i = 1:length(hends)
		r = get(hends(i),'userdata');
	...
	rx = get(s.hend,'xdata');
	ry = get(s.hend,'ydata');
	[rlat,rlon] = map.crs.internal.minvtran(rx,ry);
	units = popupstr(gcbo);
	switch units
		case 'Kilometers'
...
case 'translate'
	...
	[s.clat,s.clon] = map.crs.internal.minvtran(x,y);
	[s.rlat,s.rlon] = reckon(s.trk,s.clat,s.clon,s.rad,s.az);
	[crlat,crlon] = scircle2(s.trk,s.clat,s.clon,s.rlat,s.rlon,...
							[1 0],'deg',s.npts);
	% convert geographic coordinates to map coordinates coordinates
	[rx,ry] = map.crs.internal.mfwdtran(s.rlat,s.rlon);
...
case 'resize'
	...
	[s.clat,s.clon] = map.crs.internal.minvtran(cx,cy);
	[s.rlat,s.rlon] = map.crs.internal.minvtran(rx,ry);
	[crlat,crlon] = scircle2(s.trk,s.clat,s.clon,s.rlat,s.rlon,...
							 [1 0],'deg',s.npts);
	s.rad = distance(s.trk,s.clat,s.clon,s.rlat,s.rlon); % radius
