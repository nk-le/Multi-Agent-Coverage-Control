...
object = validatestring(object, ...
    {'surface','line','linem','patch','point','text','light','none'}, ...
    'CLIPDATA', 'OBJECT', 3);
...
validateattributes(xmat, {'double'}, {'nonempty','2d'}, 'CLIPGRAT', 'XMAT')
if ~isequal(size(xmat),size(ymat))     %  Dimensional consistency
   error(message('map:validate:inconsistentSizes2','CLIPGRAT','XMAT','YMAT'))
end
   
if length(xmat) == 1            %  Scalars don't need to be clipped.
   xout = xmat;
   yout = ymat;                 % Scalars mess up algorithm too.
   return                          
end

if size(xmat,1) == 1            %  Algorithm requires column vectors
   xmat = xmat';
   ymat = ymat';
end
...
%  Make copies of original inputs for later retrieval of data.
%  This must be done before the below operations on xmat.

xsave = xmat;     ysave = ymat;

%  Mitigate possible round-off problems with near-zero data.
...
xmat = wrapToPi(xmat-breakpoint);

%  Shift points which are almost identically zero to slightly
%  positive.  This helps the wrapToPi transformation back to
...
%  process referred to above was similar (though not identical) to the
...
validateattributes(xmat, {'double'}, {'nonempty','2d'}, 'CLIPGRAT', 'XMAT')
if ~isequal(size(xmat),size(ymat))     %  Dimensional consistency
   error(message('map:validate:inconsistentSizes2','CLIPLINE','XMAT','YMAT'))
end
...
validateattributes(xvec, {'double'}, {'nonempty'}, 'CLIPPATCH', 'XVEC')
   
if ~isequal(size(xvec),size(yvec))     %  Dimensional consistency.
   error(message('map:validate:inconsistentSizes2','CLIPPATCH','XVEC','YVEC'))
end

if any(isnan(xvec) | isnan(yvec))          %  Patches must be separated with single NaNs.
   error('map:clipdata:neighboringNaNs','Neighboring NaNs in original data.')
end

if isscalar(xvec)           %  Scalars don't need to be clipped.
   xout = xvec;             %  Scalars mess up algorithm too.
   yout = yvec;   
   clipped = 0;
   return                             
end
...
% against each other are then joined to form fillable patches. The result
...
if isempty(cindx); return; end
if mod(length(cindx),2) ~= 0 
   error('map:clipdata:invalidPatch','Problem clipping patch.') 
end
...
if length(indx) > 1
   error('map:clipdata:invalidLength', ...
       'Can''t clip patches that spiral or encompass both poles.')
end
