classdef DataLogger < handle
    ...
    methods
        function obj = DataLogger(CONST_PARAM, startPose, vList, wList)
            ...
            obj.maxCnt = obj.CONST_PARAM.MAX_ITER + 10;
            obj.PoseAgent = zeros(obj.CONST_PARAM.N_AGENT, 3, obj.maxCnt);
            obj.PoseVM = zeros(obj.CONST_PARAM.N_AGENT, 2, obj.maxCnt);
            obj.ControlOutput = zeros(obj.CONST_PARAM.N_AGENT, obj.maxCnt);
            obj.CVT = zeros(obj.CONST_PARAM.N_AGENT, 2, obj.maxCnt);
            obj.V_BLF = zeros(obj.CONST_PARAM.N_AGENT, obj.maxCnt);
        end
        ...
        function plot_Lyapunov(obj)
            botColors = cool(obj.CONST_PARAM.N_AGENT);
            xAxis = 1:obj.curCnt;
            V = zeros(1, obj.curCnt);
            % Plot
            figure
            hold on; grid on;
            for i = 1:obj.CONST_PARAM.N_AGENT
        ...
        function plot_VM_trajectories(obj)
            ...
            figure
            hold on; grid on;
            for i = 1: size(obj.CONST_PARAM.BOUNDARIES_VERTEXES,1)-1                
               plot([obj.CONST_PARAM.BOUNDARIES_VERTEXES(i,1) obj.CONST_PARAM.BOUNDARIES_VERTEXES(i+1,1)],[obj.CONST_PARAM.BOUNDARIES_VERTEXES(i,2) obj.CONST_PARAM.BOUNDARIES_VERTEXES(i+1,2)], '-r', 'LineWidth',4);                    
            end   
            for i = 1:obj.CONST_PARAM.N_AGENT
                dataVMX = zeros(obj.CONST_PARAM.N_AGENT, obj.curCnt);
                dataVMY = zeros(obj.CONST_PARAM.N_AGENT, obj.curCnt);
        ...
        function visualize(obj)
            env = MultiRobotEnv(obj.CONST_PARAM.N_AGENT);
            %% Parse data
            spX = obj.CVT(:,1,:);
            spY = obj.CVT(:,2,:);
            vmX = obj.PoseVM(:,1,:);
            ...
            poseInit = zeros(3, obj.CONST_PARAM.N_AGENT);
            env((1:obj.CONST_PARAM.N_AGENT), poseInit);
            hold on; grid on; %axis equal

            % Plot Boundaries
            xrange = max(obj.CONST_PARAM.BOUNDARIES_VERTEXES(:,1));
            yrange = max(obj.CONST_PARAM.BOUNDARIES_VERTEXES(:,2));
            offset = 20;
            ...
            for loopCnt = 1:obj.curCnt    
                if(loopCnt == 1) % Plot only once
                        ...
                        % Color Patch
                        verCellHandle = zeros(obj.CONST_PARAM.N_AGENT,1);
                        cellColors = cool(obj.CONST_PARAM.N_AGENT);         
                        if(showColorPlot == true)
                            vmPlotColorHandle = [];
                            spPlotColorHandle = [];
                            for i = 1:obj.CONST_PARAM.N_AGENT % color according to
                ...
                env((1:obj.CONST_PARAM.N_AGENT), pose);
                ...
                if(showColorPlot == true)
                    ...
                    str = str + newline + "x: WMR's Virtual Mass, o: Centroid of Voronoi Partition";
                    title(str);
                    for i = 1:obj.CONST_PARAM.N_AGENT % update Voronoi cells
