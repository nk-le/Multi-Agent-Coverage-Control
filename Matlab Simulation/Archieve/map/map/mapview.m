...
%   to project latitudes and longitudes to your desired map coordinate
%   system before you import it.  When starting from a different
%   projection, you must first un-project to latitude and longitude using
%   PROJINV, then re-project with PROJFWD.  You might also need to adjust
%   the horizontal datum of your data using, for example, the free GEOTRANS
%   (Geographic Translator) application from the Geospatial Sciences
%   Division of the U.S. National Geospatial- Intelligence Agency (NGA).
%   If you simply need a change of units, multiply by the appropriate
%   conversion factor obtained from UNITSRATIO.
%
...
%   the Insert menu.  Use the "Select annotations" tool and Edit menu to
%   modify or remove your annotations.  The Map Viewer manages annotations
%   separately from data layers; annotations always stay on top.  Note that
%   annotations cannot be saved, although they appear on a map exported to
%   an image format, as described below.
%
...
%   See also GEOSHOW, MAKESYMBOLSPEC, MAPSHOW, READGEORASTER,
%            SHAPEREAD, UPDATEGEOSTRUCT, WORLDFILEREAD
