function struc = readfields(filename,field,recordIDs,machineformat, fid)
%READFIELDS Read fields or records from fixed format file
%
% READFIELDS will be removed in a future release.
%
...
elseif nargin == 5
    ...
    warning(message('map:removed:readfieldsSparseOption', ...
        'READFIELDS','READFIELDS'))
end

validateattributes(filename, {'char'}, {'nonempty'}, 'readfields', 'FILENAME', 1)
validateattributes(field, {'struct'}, {'nonempty'}, 'readfields', 'FILESTRUCT', 2)
...
if nargin <= 2
    ...
    struc = readcols(fid,field,selectfields,filewasopen); % Fixed length records, all
elseif nargin > 2 && iscell(recordIDs)
    struc = readcols(fid,field,recordIDs,filewasopen); % Selected fixed length fields
else
    struc = readrows(fid,field,recordIDs,filewasopen); % Selected fixed length records
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function struc = readcols(fid,field,selectfields,filewasopen)
...
if ~filewasopen && mod(eof-datastartpos,recordlen) ~= 0
    ...
    if ~isempty(CRindx) && all( diff(diff(CRindx))==0 )
        ...
        fclose(fid);
        error(message('map:fileio:inconsistentRecordLength', ...
            recordlen,  actualrecordlen))
    end
...
for i=1:length(field)
    ...
    if ~isempty(field(i).name) % don't save data with empty fieldnames
    ...
    switch thisfield
                        ...
                        for k = 1:field(i).length
                            ...
                            if size(datacol,1) ~= size(strng2,1)
                                ...
                                if ~isempty(notemptyindx)
                                    ...
                                    error(message('map:fileio:inconsistentFileStruct', i))
                                end
                            end
        ...
        otherwise % numeric
            ...
            if ismember(i,[selectfields{:}]) && ~isempty(field(i).name)
                data = fread(fid,inf,[num2str(field(i).length) '*' field(i).type], recordlen-fieldlen);
...
if ~isempty(carray)
    ...
    % as a structure
    selected = ~isnan([selectfields{:}]) & fieldsfull([selectfields{:}]);
    struc = cell2struct(carray, {field([selectfields{selected}]).name}, 2);
else
    struc = [];
end
...
function struc = readrows(fid,field,recordIDs,filewasopen)
...
if ~filewasopen
    if mod(eof-datastartpos,recordlen) ~= 0
        ...
        fclose(fid);
        error(message('map:fileio:expectedEvenNumberOfRecords', ...
            num2str(eof-datastartpos), num2str(recordlen)));
    end
end
...
for k=recordIDs
    ...
    if ftell(fid) >= eof || status == -1 
        error(message('map:fileio:recordNotFound'));
    end
                    ...
                    otherwise
                        ...
                        if ~isempty(field(i).name)
                            ...
                            if count ~= field(i).length  % Didn't read as many values as expected, so there may be empty elements in the field.
                                ...
                                num = [];
                                bytes = field(i).bytes;
                                for l = 1:field(i).length % Read each element. At least we gave the vectorized method a shot
                                    strng2 = strng(:,(l-1)*bytes+(l-1)+(1:bytes));
    ...
    switch field(i).type
        ...
        otherwise
            if ~isempty(findstr('%',field(i).type)) % SCANF-style formatted data
                ...
                onefieldlen = str2double(precision(numericCharindx));
