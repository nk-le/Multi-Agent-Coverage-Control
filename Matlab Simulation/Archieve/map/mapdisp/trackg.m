...
%
%  TRACKG(n,'LineSpec') and TRACKG(n,npts,'LineSpec') uses any valid
%  LineSpec to display the great circles.
%
...
%
%  TRACKG('track',...) uses the track value to define
...
if isempty(ntrak)
    ...
    validateattributes(ntrak, {'double'}, ...
        {'real','positive','scalar','integer'}, 'TRACKG', 'N')
end
...
if isempty(str)
    ...
    str = validatestring(str, {'gc','rh'}, 'TRACKG', 'TRACK', 1);
end
...
for i = 1:ntrak
       ...
       if isempty(latpts)
           warning(message('map:trackg:emptyLATPTS'))
           return
       end
...
case 'reproject'
	...
	if ~isempty(htrack)
		for i = 1:length(htrack)
			...
			[trklat,trklon] = track2(s.trk,s.slat,s.slon,s.elat,s.elon,[1 0],...
									 'deg',s.npts);
			% update the track 
			[trkx,trky,trkz,t] = map.crs.internal.mfwdtran(trklat,trklon,zeros(size(trklat)),'line');
			s.clipped = t.clipped;
			s.trimmed = t.trimmed;
	...
	if ~isempty(hstart)
		for i = 1:length(hstart)
			...
			[x,y] = map.crs.internal.mfwdtran(s.slat,s.slon); z = 0;
			% slip in the x and y data
	...
	if ~isempty(hend)
		for i = 1:length(hend)
			...
			[x,y] = map.crs.internal.mfwdtran(s.elat,s.elon); z = 0;
			% slip in the x and y data
...
case 'startmove'
	...
	y = cpoint(1,2);
	% starting point
	[s.slat,s.slon] = map.crs.internal.minvtran(x,y);
	% other properties
	s.rad = distance(s.trk,s.slat,s.slon,s.elat,s.elon); % radius
	s.saz = azimuth(s.trk,s.slat,s.slon,s.elat,s.elon); % starting azimuth
	s.eaz = azimuth(s.trk,s.elat,s.elon,s.slat,s.slon); % ending azimuth
...
case 'endmove'
	...
	y = cpoint(1,2);
	% ending point
	[s.elat,s.elon] = map.crs.internal.minvtran(x,y);
	% other properties
	s.rad = distance(s.trk,s.slat,s.slon,s.elat,s.elon); % radius
	s.saz = azimuth(s.trk,s.slat,s.slon,s.elat,s.elon); % starting azimuth
	s.eaz = azimuth(s.trk,s.elat,s.elon,s.slat,s.slon); % ending azimuth
