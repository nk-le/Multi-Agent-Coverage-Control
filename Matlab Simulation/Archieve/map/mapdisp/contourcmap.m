...
%   with the colormap specified by CMAPSTR. Valid entries for CMAPSTR
%   include 'pink', 'hsv', 'jet', or any similar MATLAB colormap function.
%   If the axes contains Mapping Toolbox contour objects, the resultant
%   colormap contains the same number of colors as the original colormap.
%   Otherwise, the resultant colormap contains 10 colors.
% 
...
%   -------------------
%  
%   'Colorbar'          Either 'on' or 'off' specifying whether a colorbar
%                       is present, 'on', or absent from the axes, 'off'.
%
%   'Location'          The location of the colorbar.Permissible values are 
%                       'vertical' (the default), 'horizontal', or 'none'.
%
%   'ColorAlignment'    The alignment of the labels in the colorbar. 
%                       Permissible values are 'center', where the labels
%                       are centered on the color bands or 'ends' where
%                       labels are centered on the color breaks. If the
%                       axes contains Mapping Toolbox contour objects, the
%                       ColorAlignment will be set automatically to
%                       'center' for contour lines and 'ends' for filled
%                       contours, and cannot be modified.
%
...
%
%   'TitleString'       The title of the colorbar axes.
%
%   'XLabelString'      The X label of the colorbar axes.
%
%   'YLabelString'      The Y label of the colorbar axes.
%
%   'ZLabelString'      The Z label of the colorbar axes.
%                        
...
%   contourcmap('summer',2000,'Colorbar','on','Location','horizontal', ...
%      'TitleString','Contour Intervals in Meters');
...
% Validate CMAPSTR
validateattributes(cmapstr, {'char','string'}, {'nonempty','scalartext'}, ...
    mfilename, 'CMAPSTR', pos(1));
map.internal.assert(exist(cmapstr,'file') == 2, ...
...
map.internal.assert(mod(numel(params),2) == 0, ...
...
map.internal.assert(ishghandle(h), ...
    'map:contourcmap:invalidHandle','SourceObject')
...
set(hndl,'DeleteFcn',@deleteColormapListener)
    ...
    function updateColors(~,~)
        ...
        % Reset/restore colorbar axes properties as needed.
        if fill
...
elseif isempty(cdelta)
	...
	tolerance = 100*eps(max(abs(cdelta(:))));
